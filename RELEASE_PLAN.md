...
| L1.3 | Create a Cloudflare R2 bucket, set `R2_*` | **Owner** | ✅ DONE 2026-08-13 — bucket `adgenwebsaas`, **EU jurisdiction** (the Privacy page promises EU-resident data), token scoped to that one bucket, public dev URL enabled. ✅ LIVE-VERIFIED: a real Lambda render was uploaded to it and plays from the r2.dev url. **One code fix it forced:** the S3 endpoint was derived as `<account>.r2.cloudflarestorage.com`, which is only right for a DEFAULT-jurisdiction bucket — an EU bucket needs `<account>.eu.…` and would have failed every request with "bucket not found" (fixed @ `7c7a1fd`, `R2_ENDPOINT`). **Still on r2.dev**, which Cloudflare rate-limits and does not intend for production — swap to `cdn.<domain>` once the domain exists. Original note: `storage.r2.ts` is written and typechecks but **has never made a single real call**. Until this exists, rendered files have nowhere durable to live, and `enhance`/`remove_text` are hard-blocked — they refuse `localhost` source URLs because fal.ai cannot fetch them. |
...
| L2.3 | **Remotion Lambda — ✅ LIVE 2026-08-13** | Owner did the AWS setup, Claude drove the deploys | ✅ DONE: IAM user + role (`remotion-lambda-role`) with Remotion's own generated policies, function `remotion-render-4-0-490-mem2048mb-disk2048mb-120sec` and site `adgen` both deployed to `eu-central-1`. **First real render: 26.8s** for a 5s clip through our own `RemotionLambdaRenderer` — the mp4 was fetched, uploaded to R2, and the AWS copy deleted, so the ownership transfer is proven rather than assumed. `@remotion/lambda@4.0.490` had to be added (only `lambda-client` was installed; the CLI package is what deploys) — it is pinned to match, since a version drift between function, CLI and client is the classic Lambda failure. Owner's original call: the product is meant to serve many users, so rendering goes serverless and the VPS keeps the web app, Redis and yt-dlp. My recommendation had been "later, when bursts hurt"; the owner reaffirmed, so it is the plan. **Sequencing matters: R2 (L1.3) must come FIRST.** Lambda without R2 renders in the cloud and then copies the file to the worker's local disk, which is the worst of both. See the runbook below. **Code hardening 2026-08-12 (`d28a20f`):** the render timeout is now progress-aware (no longer fails a slow-but-advancing paid render) and the ownership fetch retries transient 5xx/network blips — two of three open findings closed. **The third stays for the owner's AWS run:** the finished output is still `privacy:'public'` for the few-seconds window before the delete; tightening it to `privacy:'private'` + a presigned fetch needs a live AWS run to verify, so it was deliberately not guessed at. |
...
| L2.5 | Capture real per-job cost against provider dashboards | **Owner reads the invoice, Claude did the rest** | ✅ instrumented @ `a3fc8cf`. This could not be measured before because a job had no cost CEILING: the 700-char cap applied only to human-reviewed scripts, a model-written one went to ElevenLabs verbatim, and the render duration was unclamped. Both limits now live in core (`MAX_SCRIPT_CHARS` 700, `MAX_AD_SECONDS` 60), so a full `count=15` job is bounded at **10,500 spoken characters and 900 seconds of video** — that is the ceiling on the provider bill. The worker now logs `job spend` per job: characters, variants, render seconds, video seconds, plus which voice provider and renderer were used. Units, not money — rates differ per plan and change, so the multiplication is the owner's. **What is left: run a handful of real jobs, then compare the logged units against the actual ElevenLabs and OpenRouter invoices.** That is the number `BUSINESS.md` has always modelled and never measured. |
...
| L4.5 | Close the SSRF gaps | Claude | ✅ done @ `8868836` — the routes now call `assertPublicHost`, which RESOLVES the hostname and rejects if any returned address is private (IPv4 and IPv6, including `::ffff:` IPv4-mapped and CGNAT). The redirect gap turned out not to be live: `scraper.real.ts` already fetches with `redirect: 'manual'` and throws on any 3xx. **Remaining and undefendable here: DNS rebinding** — the name is resolved by this check and again by whatever fetches, and neither `fetch` nor the yt-dlp binary lets us pin the resolved IP. Raised the bar a long way; not airtight. |
