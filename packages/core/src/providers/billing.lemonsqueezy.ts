...
export class LemonSqueezyBilling implements Billing {
      ...
      body: JSON.stringify({
        data: {
          ...
          attributes: {
            ...
            product_options: {
              redirect_url: `${this.config.appUrl}/app?kupljeno=1`,
            },
          },
          relationships: {
  ...
  async parseWebhook(req: Request): Promise<{ userId: string; amount: number; reason: string; orderId: string } | null> {
    ...
    // G1: never trust custom_data.pack_id on its own. A wrong/stale
    // LEMONSQUEEZY_VARIANT_MAP entry — or any path where the custom data and
    // the real purchase disagree — would grant a larger pack than was paid for.
    // Lemon Squeezy carries the actually-purchased variant as a NUMBER at
    // data.attributes.first_order_item.variant_id; compare it as a string
    // against the variant mapped to this pack.
    const purchasedVariantId = payload.data.attributes.first_order_item?.variant_id;
    if (purchasedVariantId !== undefined) {
      const expectedVariant = this.variantMap[packId];
      if (String(purchasedVariantId) !== expectedVariant) {
        console.warn(
          `[billing] Refusing credit grant for pack "${packId}": paid variant ` +
            `${purchasedVariantId} does not match the mapped variant ` +
            `${expectedVariant ?? '(none)'}. A wrong LEMONSQUEEZY_VARIANT_MAP entry ` +
            `could hand out a larger pack than was paid for.`,
        );
        return null;
      }
    } else {
      console.warn(
        `[billing] Paid order for pack "${packId}" carried no purchasable variant_id ` +
          `in the webhook payload — granting credits without the cross-check (shape never seen live).`,
      );
    }
