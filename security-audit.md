...
| 2   | **NEW: 6to4 `2002::/16` embedded-private-v4 reads public**                                                                                        | 🟠       | §1; probe-verified; needs host to route 6to4 (unlikely on Hetzner) so practical risk low — but fifth in the family; two-line fix given |
| 3   | Teredo `2001:0::/32` + operator NAT64 prefixes read public                                                                                        | 🟡       | §1 NOTEs; no legitimate product case for Teredo-to-private; block-whole-prefix is the simple call                                      |
| 4   | 0011 **not yet applied**; until then the double-charge window is open, widened by the `spawnSync` lock-lapse aggravator (needs only a slow video) | 🟠       | Owner action: run the reconciliation query, apply, verify the zero-rows check                                                          |
| 5   | "Charged once, error, nothing" state after 0011 (§2a nuance) + no re-entry guard (§2b)                                                            | 🟠       | One worker change fixes both: ledger-row-exists ⇒ rebuild-and-done                                                                     |
| 6   | CI scanning absent                                                                                                                                | 🟡       | Exact file delivered §3                                                                                                                |
