...
| 6   | ~~CI scanning absent~~ — **ADDED 2026-08-18**                                                                                                     | 🟡       | `e3ddd57` + `d4f67b2`. Found 26 advisories on its first run, 2 of them false (Remotion criticals naming `<4.0.410` against a lockfile holding 4.0.490, printing an EMPTY dependency path — ignored via `pnpm.auditConfig.ignoreGhsas` with the reason written down). Runtime-path advisories fixed by pinned overrides, **`undici` being the one that mattered**: cheerio fetches customer-supplied URLs with it. **CLEAN as of 2026-08-18** (`c638543`): `pnpm audit --audit-level high` reports `2 critical (2 ignored)` — nothing but the two suppressed Remotion entries. Getting there took two steps and the first was a false victory worth remembering: upgrading `vitest` 2.1.9 → 3.2.7 closed only the vitest finding, because vitest 3 declares `vite: ^5 || ^6 || ^7` and pnpm kept the lockfile's existing vite 5.4.21 — the three vite advisories and esbuild did not move. An override forcing `vite: ^7.0.0` is what actually closed them (vite 7.3.6; esbuild 0.28.1 is now the only one in the tree). Zero test changes were needed for either step. One side effect had to be handled: vitest 3's cold transform is slower (9.92 s vs 2.87 s measured), which pushed `factory.test.ts`'s empty-env case past the 5 s default on a cold cache while it passed warm at 2438 ms — invisible locally, guaranteed on CI, which compiles cold every run. `packages/core` gained its first vitest config for that one setting. The `typescript-eslint` half was closed the same day by Cline (`0d1b3f6`) and needed no upgrade at all: `js-yaml` 4.3.1 and `brace-expansion` 1.1.18/5.0.9 were already inside the ranges their parents declare, so the lockfile had simply frozen older ones. Both `brace-expansion` MAJORS had to stay — `minimatch@3` wants `^1`, `minimatch@10` wants `^5` — and collapsing them into one is the obvious way to break this. ⚠️ Ordering flaw found by the job's own first run: `pnpm audit` ran FIRST, failed, and GitHub SKIPPED gitleaks and CodeQL — a dependency backlog silencing the scanners that matter. Audit is last now |
