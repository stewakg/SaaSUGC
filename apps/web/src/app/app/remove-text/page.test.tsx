...
/**
 * Tests for the Remove-text wizard PAGE (F5) — /app/remove-text.
 *
...
import RemoveTextPage from './page';
...
const COST_LABEL = creditsLabel(getJobDescriptor('remove_text').cost);

const fetchMock = vi.fn();

describe('RemoveTextPage', () => {
  ...
  it('a successful upload makes the wizard advanceable', async () => {
    ...
    mocks.uploadFile.mockResolvedValue({
      url: 'https://files.example.com/spot.png',
      name: 'spot.png',
    });
    await dropFile(container, 'spot.png', 'image/png');
    expect(mocks.uploadFile).toHaveBeenCalledTimes(1);
    expect(container.textContent).toContain('Otpremljeno: spot.png');
  ...
  it('a 402 from /api/jobs surfaces as an alert instead of the wizard hanging', async () => {
    ...
    expect(container.textContent).not.toContain('Uklanjam tekst');
