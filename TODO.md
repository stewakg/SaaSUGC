...
| ❌ | **User can delete own files — owner wants it (2026-08-18)** | 🤖 | Wire the existing `Storage.delete` (§ row below) to a user-facing action: a delete control in „Moje reklame" → route verifies ownership (same `authorise` discipline as `/api/storage`), deletes the R2 objects by `storage_key`, removes/patches the asset rows and `jobs.result`. This is ALSO the building block GDPR erasure calls per-file. Scope note: this row is per-file deletion; account-wide erasure keeps the open decisions in §3b (ledger rows are accounting records, mid-render jobs) |
