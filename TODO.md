...
| ✅ | **Production is at `c0d0a19`** | — | Deployed 2026-08-20 from the primary machine. Carries the money work of that session: the `enhance` input cap (§2a's ⛔, now closed), the re-priced credit packs (25% under the competitor), the quiet price display, the "Spona" step rail and the split auth pages. **Verified inside the containers rather than by `healthy`:** worker `src/pipelines.ts` has `probeVideoMeta`/`planEnhanceVideo`, its bundled `@adgen/core` reads `ENHANCE_MAX_SECONDS = 30`, and it logs `listening` on both queues with `mediaEdit: fal-media-edit` — the provider the cap exists to protect. Web image carries `4,50` + `po kreditu` in the compiled profil page and `auth-brand`/`progress--thick` in the stylesheet. HTTP `/` 200 · robots 200 · traversal **401** · `/login` 200, and the public site renders the new login. Queues were empty before the deploy; 6.74 GB of build cache pruned after, disk 47% → 44%. Previous deploy: |
