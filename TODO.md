...
| ⛔ | **`REDIS_PASSWORD` must be added to `/srv/adgen/.env` BEFORE the next deploy** | 👤 | Checked 2026-08-17: the box does not have it. The compose file now requires it, so a deploy without it fails at `config` time — safely, but it fails. Generate with `openssl rand -base64 32`, add the line, then deploy. This is the one item where the fix and the deploy cannot be done in either order |
...
ssh root@5.75.154.153 'cd /srv/adgen && docker compose -f infra/docker-compose.prod.yml -p adgen ps'
```
