...
| ❌ | **Server-log retention ≤30 days — `/privatnost` §4 now claims it** | 👤 | Added 2026-08-18 with the GDPR pass (a retention period for logs must be STATED, and now it is). Docker's default `json-file` driver keeps logs forever — add rotation on the VPS (`logging: options: max-size/max-file` in `infra/docker-compose.prod.yml`, or daemon-wide in `/etc/docker/daemon.json`) so the claim is true |
