...
| 🔄 ⛔ | **AWS Lambda concurrency quota — increase REQUESTED 2026-08-16, pending** | 👤 waits, then 🤖 tunes | Measured, not guessed: `get-service-quota` returns **`Value: 10.0`** in eu-central-1 — the new-account default, and the reason the first full-chain run could not render a ten-second ad at all. One render costs `concurrency + 1` executions (chunk Lambdas + launcher), so at today's cap of 3 that is **4 per render: two customers at once already sit at 8 of 10, and the third fails.** Increase to 1000 requested via CloudShell, request id `808f14502cca4879991991ce7168b4b6MKs29MiW`, status PENDING. Check it with `aws service-quotas get-requested-service-quota-change --request-id <id> --region eu-central-1`. **When it lands:** raise `REMOTION_LAMBDA_CONCURRENCY` in `/srv/adgen/.env` and restart the worker — env only, no rebuild — and measure one render at 3 vs ~25 before settling on a number. Note the credentials the app holds cannot read this quota (`lambda:GetAccountSettings` is denied to the `remotion` IAM user, deliberately), so this check is always a console/CloudShell job |
