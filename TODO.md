...
**Updated 2026-08-22.** One item off this list closed on the app side and blocked on the bucket side: **§5's 30-day retention ⛔**. The rules, the 410 handling and the „Moje reklame" states are live in the repo (`31189ec`, tests 1238 → **1283**), but the R2 lifecycle rule itself is NOT applied — the token cannot write it (`AccessDenied`), so the retention sentence in `/uslovi` and `/privatnost` remains false until the owner acts. See the §5 row for the two ways to close it. The previous entry:
...
| ✅ | **AWS Lambda concurrency quota — APPROVED 2026-08-16, now 1000** | 🤖 tunes, later | AWS approved the increase the same day: concurrent executions in eu-central-1 went **10 → 1000**. ⚠️ **Nothing has been changed or tested yet — the owner asked to hold off.** When the time comes: raise `REMOTION_LAMBDA_CONCURRENCY` in `/srv/adgen/.env` and restart the worker (env only, no rebuild), and measure one render at the current 3 vs ~25 before settling on a number. Remember the ceiling below is now the box, not AWS. ⚠️ **Do not invoke it as `pnpm r2:lifecycle` from the repo root** — the root script shells out to `pnpm --filter …` and corepack runs pnpm 11 against a `packageManager` pinned to 10, which refuses to switch versions (`db:seed` has the same shape; not this feature's bug). Run it directly: `cd apps/worker && ./node_modules/.bin/tsx --env-file-if-exists=.env scripts/r2-lifecycle.ts`. Original row: |
...
| ✅ | **Production is at `cdabed3`** | — | Deployed 2026-08-20, second deploy of the day. Carries the 45-credit signup bonus (so "3 besplatna videa" is finally payable), the 60-credit/€9 starter pack, `enhance` billed per 30s tier up to 120s, the ai_video fal fallback switched off, and the step-rail thread removed. Verified inside the containers: worker has `underpaid_duration` in `src/pipelines.ts`, `ENHANCE_MAX_TIERS = 4` in its bundled core, and `enhanceTiers` in `job-state.ts`; web has `enhanceTiers` in the compiled `api/jobs/route.js`. HTTP / 200 · robots 200 · traversal **401** · /signup 200, and the public landing and signup pages both render "3 besplatna videa". ⚠️ **The DB still grants 3 credits** until the owner applies `0012_signup_bonus_45.sql` — the copy is live before the grant is. Build cache pruned 5.13 GB. Previous deploy: |
| ✅ | **Production was at `c0d0a19`** | — | Deployed 2026-08-20 from the primary machine. Carries the money work of that session: the `enhance` input cap (§2a's ⛔, now closed), the re-priced credit packs (25% under the competitor), the quiet price display, the "Spona" step rail and the split auth pages. **Verified inside the containers rather than by `healthy`:** worker `src/pipelines.ts` has `probeVideoMeta`/`planEnhanceVideo`, its bundled `@adgen/core` reads `ENHANCE_MAX_SECONDS = 30`, and it logs `listening` on both queues with `mediaEdit: fal-media-edit` — the provider the cap exists to protect. Web image carries `4,50` + `po kreditu` in the compiled profil page and `auth-brand`/`progress--thick` in the stylesheet. HTTP `/` 200 · robots 200 · traversal **401** · `/login` 200, and the public site renders the new login. Queues were empty before the deploy; 6.74 GB of build cache pruned after, disk 47% → 44%. Previous deploy: |
| ✅ | **Production was at `0a4648d`** | — | Two more deploys later the same night (2026-08-19, both with the `-p adgen` ritual): `9663d60` — Uskoro sidebar page, strip-system utility cards with their own hues, and the §3a renames („Nova reklama" / „Reklama sa novim zvukom"); then `0a4648d` — full-width app layout (wizard rail hugs the sidebar and follows its collapse; reklame/profil/admin lose their centred max-width). Verified: uskoro `page.js` in the image, „Nova reklama" present in compiled chunks, HTTP / 200 · robots 200 · traversal 401 · /app/uskoro 307→login, all three containers healthy. Cache pruned 3.18 GB, disk 50%. Previous deploy: |
| ✅ | **Production was at `055d498`** | — | Deployed 2026-08-19 by Claude over SSH from the second machine — file deletion + admin panel + yt-dlp pin (its first real `docker build`: passed). Verified inside the containers, not by "healthy" alone: `.next/server/app/api/admin/users` (+`[id]`) and `app/admin/page.js` present in the image, worker re-attached on real providers, HTTP `/` 200 · robots 200 · traversal **401** · unauthenticated `/api/admin/users` **401** · `/app/admin` 307→login. **Three deploy traps hit and recorded:** the repo at `/srv/adgen` needs `git config --global --add safe.directory /srv/adgen` for root; the box's tree was full of CRLF noise + yesterday's rsync content (86 "modified" files, ZERO unique work — proven with `git diff --ignore-cr-at-eol origin/main` before a `reset --hard`); and compose MUST get **`-p adgen`** — without it a parallel `infra_*` project spawns with an EMPTY redis volume and name-conflicts against the real containers (cleaned up same minute). Build cache pruned after: 8.86 GB, disk 57% → 35%. Previous deploy: |
| ✅ | **Production was at `4aaa761`** | — | Second deploy of 2026-08-18, carrying the `--prod` worker image and the NAT64 local-use fix. Verified INSIDE the running containers, not by "healthy": worker logs `listening` on both queues with real providers (which is what proves `tsx` survived `--prod` — had it been pruned the container would not start at all), `vitest` is **0** and `tsx` is **1** in the live image, the NAT64 constant is present in the compiled web chunk, and the runtime triple still answers `/` 200, `/robots.txt` 200, traversal **401**. Worker image 4.60 → 4.44 GB. Build cache pruned after: 2.78 GB, disk 46% → 36%. Previous deploy the same day: |
| ✅ | **Production was at `2484475`** | — | Deployed 2026-08-18 by Claude over SSH from the SECOND machine (the VPS key was already there, which `TODO.md` §8 warns is machine-local — it happened to be present). Both images rebuilt, all three containers `healthy` within 20s. **This deploy carried the money fix**: the re-entry guard, plus the Teredo SSRF branch, the storage traversal guard, and new dependency versions (undici 7.29, sharp 0.35.3, postcss 8.5.26) — which is why the build was long, it re-installed rather than re-compiled. Verified, not assumed: `/` and `/robots.txt` 200; all ten API routes present in `.next/server/app/api` including `storage` (the route a `.dockerignore` pattern once hid from every image ever built); the guard's own log strings found INSIDE the worker container; the NAT64 (`65435`) and Teredo (`8193`) constants both present in the compiled web chunk; and an unauthenticated traversal answering **401 rather than 400**, which is the discriminator proving the signing branch is live AND that the new guard runs after `authorise` as designed. Build cache pruned afterwards: 9.03 GB, disk 55% → 33%. Previous deploy: |
...
| Status | Item | Who | Note |
|---|---|---|---|
| ✅ | **`enhance` is billed per 30s tier — DONE 2026-08-20** | — | The cap became a price. `enhance-limits.ts` bills `ceil(seconds / 30)` tiers at 9 credits each, up to 4 tiers = **120s** (was a flat refusal above 30s, which protected the margin by making the tool useless for a 45-second ad). Margin is now ~39% worst-case at EVERY length instead of decaying with duration. Three places enforce it: the wizard measures in the browser and shows the price, `/api/jobs` prices from that duration and writes `params.enhanceTiers` as a receipt, and the worker re-measures with ffprobe and refuses `underpaid_duration` before spending. `job-state.ts` charges tiers × unit for enhance rather than unit × asset count. Previous row: |
...
| ✅ | **`ai_video` fal fallback — CLOSED in code 2026-08-20** | — | `KieAIFalRouter.generateVideo` no longer falls back automatically: the fallback is opt-in (`allowVideoFallback`, default false) and a kie failure now fails the job with a message saying it was not charged. Images keep their automatic fallback — $0.04 either vendor. What remains is a PRICING question for whenever ai_video ships: pay for availability by raising its credit price, or give the fallback its own ceiling. Two tests pin both directions of the switch. Original row: |
...
| ✅ | **Production worker image no longer ships a test runner — DEPLOYED 2026-08-18** | — | Live at `4aaa761`. The `--prod` install was the one change that could have broken the container outright rather than degrading it, so it was proved twice: once by a throwaway test build before committing, and again in the RUNNING container after the deploy — worker logs `listening` on both queues (so tsx loaded), and the package census inside the live image reads **vitest 0, tsx 1**. Image 4.60 → 4.44 GB. Original row: |
| ~~🟡~~ | ~~**…BUILD-VERIFIED, not yet deployed**~~ | — | `3119886`. `tsx` moved to `dependencies` (it genuinely runs in production — the CMD is `node --import tsx src/index.ts`) and the install became `--prod`. `typescript` did NOT have to move: tsx transpiles through its own bundled esbuild. **Verified by an actual build on the VPS under a throwaway tag, in a `/tmp` clone, with the running containers untouched** — because typecheck and 131 tests exercise no Docker layer at all, and a `--prod` that prunes something the container needs fails only at startup. Two probes inside the image: importing the real `src/health.ts` through the tsx loader returned the actual `startHeartbeat` function (the same probe pattern used when the CMD was last changed), and the package census reads **vitest 0, typescript 0, tsx 1**. Image 4.60 → 4.44 GB. Test image, clone and 1.62 GB of build cache cleaned up afterwards; disk back to 33%. ⚠️ **Production still runs the OLD image** — this only takes effect on the next deploy |
...
| 🟡 | **Admin panel — /app/admin (owner asked 2026-08-19)** | **BUILT + DEPLOYED 2026-08-19; credit adjustment VERIFIED live the same night** — the owner opened the page on production and confirmed "krediti rade" (add/subtract through the real `add_credits` RPC against the live DB). Account DELETION remains never-executed (no expendable account to try it on yet). Original details: every account with email + balance + created_at, manual credit add/subtract through the `add_credits` RPC as ledger reason `admin_adjust` (bounded ±100 000/call), manual account deletion (storage objects first, then `auth.admin.deleteUser` cascading profiles→jobs/assets/ledger/holds; refuses self-deletion and accounts with queued/running jobs). Gate is `ADMIN_EMAILS` server-side on page AND both API routes, NO dev carve-out, non-admins get 404. No nav link on purpose — type the URL |
...
## 3d. Glas — sređivanje i kloniranje (owner, 2026-08-20)

| Status | Item | Who | Note |
|---|---|---|---|
| 🟡 | **Glasovi: dva „topshop" glasa umesto ponude od 58** | 👤 bira, 🤖 radi | **ODLUKA VLASNIKA 2026-08-22: ne kurirati 58 glasova, nego ponuditi SAMO DVA** — muški i ženski „topshop" glas, energični reklamni ton kakav ima konkurencija — plus klon kupca kao treću opciju. Manje izbora, jasnija ponuda, i nijedan kupac ne bira glas koji zvuči loše. **Šta je već urađeno u tom pravcu (2026-08-22):** provajder sada provlači `language`, `accent`, `age`, `use_case` i `verified_languages` iz ElevenLabs-a umesto da ih baci, a `packages/core/src/voices.ts` rangira glasove po tome koliko su blizu maternjem srpskom (`native` / `verified` / `foreign`) i daje srpski opis. To NIJE suvišno sa dva glasa — to je alat kojim se ta dva biraju, i garancija da podrazumevani glas nije engleski. **Merenje koje je do toga dovelo:** od 58 glasova u nalogu samo 5 su naši (Ana, Milance, Milovan, Zlata S, Slobodan); 38 su engleski, 10 nemački — a kupac koji izabere „Adam - American" dobija srpsku reklamu sa američkim naglaskom i nigde mu to ne piše. **Ostaje:** poslušati te glasove i izabrati dva (previews postoje u R2 pod `previews/`), po potrebi ih napraviti kloniranjem ako nijedan postojeći ne odgovara topshop tonu, skratiti picker na njih, i obrisati nekorišćene glasove iz naloga da se oslobode IVC slotovi. Prvobitni opis problema: 58 ElevenLabs glasova ide u picker bez ikakvog reda — nisu filtrirani po jeziku, nisu grupisani po polu ni po tonu, i imena su engleska (`Rachel`, `Adam`) što srpskom kupcu ne znači ništa. Previews postoje od 2026-08-19 (▶ pored pickera, mp3 u R2 pod `previews/`). Treba: kratka kurirana lista umesto svih 58, srpska imena/opisi („ženski, topao, za kozmetiku"), i podrazumevani glas po alatu. Odluka koja fali: koliko glasova ostaje u ponudi |
| 🟡 | **Kloniranje glasa — pravni deo NAPISAN, kod nije** | 👤 pa 🤖 | **Istraženo 2026-08-20 iz ElevenLabs dokumenata; uslovi (§6) i privatnost (§3a) dopunjeni, migracija `0013_voice_clones.sql` napisana ali NIJE primenjena.** Nalazi koji odlučuju o obliku funkcije: (1) **PVC otpada** — njihov FAQ kaže "Even with their consent, you cannot clone someone else's voice" i traži voice-captcha vezanu za fizički prisutnu osobu, što naš tok (jedan firmin nalog, naš ključ) ne podržava; ostaje **IVC**, dostupan od Creator plana naviše. (2) **Jedini gate kod IVC je čekboks, i daje ga VLASNIK NALOGA, ne krajnji kupac** — mi ElevenLabs-u izjavljujemo da imamo prava na glas koji pripada kupcu, pa je naš zapis o pristanku jedini način da ta izjava bude istinita. (3) Odgovornost je ugovorom na nosiocu naloga, uz klauzulu o obeštećenju (čl. 9) i B2B2C pravilo da svojim kupcima ne smemo dati blaže uslove nego što ih mi imamo. (4) Traže izričito objavljivanje da je sadržaj AI ("clearly and prominently disclose"). (5) Brisanje glasa i brisanje istorije su DVA odvojena poziva — `DELETE /v1/voices/{id}` ne briše generacije. (6) Zero-retention je samo Enterprise, svi podaci se prenose u SAD, a glas zovu biometrijskim tamo gde ga propis tako definiše. **IZMERENO 2026-08-20 iz API-ja (`/v1/user/subscription`), vise nije nepoznato: plan je `creator`, aktivan; `voice_limit` je **30 IVC slotova**, `voice_slots_used` **14**, dakle **16 slobodnih**; PVC 1/1 zauzet; 300k karaktera mesecno, potroseno 1.868. To je TVRDO ogranicenje broja kupaca koji istovremeno mogu imati klon, i ono odlucuje oblik funkcije vise nego cena. Od 14 zauzetih, jedan je `Ja proba` a 12 su `generated` glasovi — brisanje neiskoriscenih odmah vraca slotove.** **DVE ODLUKE VLASNIKA, obe otvorene (2026-08-20, odlozene svesno):** (a) **sta kad se 30 slotova popuni** — predlog je da klon istekne 60 dana od poslednjeg koriscenja pa se automatski obrise (slotovi kruze sami); alternative su tvrdi limit uz rucno ciscenje, ili da kloniranje dobiju samo Pro i Agency paketi; (b) **cena** — IVC se NE naplacuje po klonu (ukljucen u plan), pa je marza ~100% i cena sluzi samo da odvrati "napravicu pa necu"; predlog 25 kredita jednokratno (~EUR 2,50-3,75 zavisno od paketa). Kod ceka te odluke: metode na provideru (`cloneVoice`/`deleteVoice`), ruta sa zapisom pristanka, UI, brisanje klona kad se brise nalog, i istek ako se izabere rotacija. Kupac otpremi svoj snimak i reklame govore NJEGOVIM glasom. ElevenLabs to nudi kroz Voice Add/IVC (instant) i PVC (professional). Otvorena pitanja pre koda: (1) **cena i plan** — kloniranje je vezano za ElevenLabs plan, a naš plan još nije zabeležen (MARGINS.md §6); (2) **cena za kupca** — jednokratna naplata po klonu ili uključeno u paket; (3) **pravno** — pristanak vlasnika glasa je obavezan, uslovi korišćenja i privatnost moraju to pokriti pre prvog klona, i to su iste stranice koje već čekaju LLC; (4) **skladištenje** — klon živi kod ElevenLabs-a vezan za NAŠ nalog, pa brisanje naloga kupca mora da briše i klon (§5 file deletion je isti problem) |

## 3e. Čitljivost teksta — cela aplikacija, ne samo kartice (vlasnik, 2026-08-22)

**Odluka vlasnika:** tekst na karticama je preslab da bi se lako čitao, treba da bude podebljan i
vidljiviji — i **to verovatno važi za ceo sajt, ne samo za taj ekran**. Povod je snimak ekrana
dashboarda: naslovi kartica se čitaju odmah, ali opis, kvačice i cena deluju kao da se povlače u
pozadinu.

**Šta je tačno u kodu danas** (`apps/web/src/components/tool-cards.tsx`, `app/globals.css`):

| Element | Klase | Token | Kontrast |
|---|---|---|---|
| Naslov kartice | `text-lg font-bold text-txt-hi` | `--txt-hi` `#f2f1f7` | 17.4:1 |
| Opis + kvačice | `text-sm text-txt-mid` (težina 400) | `--txt-mid` `#aaa8b8` | 7.5:1 |
| Cena („N kredita po izlazu") | `text-xs text-txt-low` | `--txt-low` `#807e90` | **4.7:1 na 12px** |

Dakle problem nije jedna klasa — tri su poluge, i biraju se **po ulozi**, ne paušalno:
**težina** (400 → 500 za telo teksta), **token** (`txt-low` je u `globals.css` izričito označen
kao „meta text only", a nosi cenu i datume koje kupac zaista čita) i **veličina** (`text-xs` = 12px).

| Status | Item | Who | Note |
|---|---|---|---|
| ❌ | **Prolaz kroz celu aplikaciju, ne kroz jedan ekran** | 👤 odlučuje izgled, 🤖 radi | Popis svih mesta koja koriste `text-txt-mid` / `text-txt-low` i odluka po ULOZI teksta (telo, meta, cena, greška), pa jedna izmena umesto pojedinačnih zakrpa. Isti tekst se pojavljuje na dashboardu, u „Moje reklame", u svih šest čarobnjaka i na pravnim stranama |
| ❌ | **Zamka: „podebljaj sve" ubija hijerarhiju** | 🤖 | Naslovi su VEĆ `font-bold`; ako telo teksta takođe postane bold, kartica postaje ravna i ništa se ne ističe. Realan potez je `font-medium` (500) za telo + podizanje cene sa `txt-low` na `txt-mid`, a bold ostaje rezervisan za naslove |
| ❌ | **Mora se proveriti u SVE četiri teme** | 🤖 | `globals.css` definiše zasebnu `--txt-hi/-mid/-low` trojku po temi (podrazumevana tamna, druga tamna, svetla, zelena) plus `prefers-color-scheme` blok. Promena tokena koja izgleda dobro u tamnoj temi može da obori kontrast u svetloj — brojevi u tabeli gore važe SAMO za podrazumevanu tamnu |
| ❌ | **Provera na pravom telefonu, ne u emulaciji** | 👤 | Isti red kao §9 „One pass on a real phone". Čitljivost 12px sivog teksta je upravo ono što emulirani 375px viewport na desktop monitoru NE pokazuje pošteno |
...
| ✅ | **Cookie consent — no banner needed, documented** | — | Resolved 2026-08-18: the app sets exactly three first-party cookies (Supabase auth, `adgen-theme`, `adgen_tz`), all user-requested functionality, zero analytics/tracking — the consent exemption applies. `/privatnost` §6 now lists all three (**the `adgen_tz` cookie was missing from the page** — it said "tačno dva" while `profil/page.tsx:95` sets a third). Re-open ONLY if analytics or any third-party script ever lands — the page itself says consent must be arranged BEFORE, not after |
| 🟡 | **User can delete own files — owner wants it (2026-08-18)** | — | **BUILT 2026-08-19, CODE-COMPLETE** (13 tests, typecheck, build; never run against real R2): `DELETE /api/jobs/:id` — RLS ownership check, 409 while queued/running, storage objects deleted BEFORE their rows (a mid-flight failure stays retryable instead of stranding objects), rows removed service-role, `jobs.result` patched to `assets: [] + files_deleted`, „Obriši" button with confirm on every „Moje reklame" row (`delete-job-files.tsx`, 4 component tests incl. confirm-declined ⇒ no request). Deliberately NOT deleted: the raw `uploads/<uid>/…` (no DB rows, no `Storage.list()`; the 30-day lifecycle rule ages them out). Account-wide erasure still keeps §3b's open decisions |
| 🟡 ⛔ | **30-day retention — the APP half is done; the BUCKET half is blocked on an R2 token** | 👤 needs an Admin Read & Write R2 token (or six rules typed into the dashboard) | **2026-08-22, `31189ec`.** The code half is finished and VERIFIED by tests: `packages/core/src/retention.ts` is the single source of truth (30 days, the six expiring prefixes, and `previews/` deliberately on NO list — S3/R2 filters cannot say "everything EXCEPT this prefix", so a whole-bucket rule would have deleted all 58 voice previews), a guard test SCANS the real source files so a new prefix nobody classified turns the suite red, `S3CompatibleStorage` gained get/putLifecycleRules, `/api/storage` answers **410 Gone** for an expired asset row or an aged upload key instead of redirecting to a signed url that 404s, and „Moje reklame" stops offering the link and shows a final-week countdown. `apps/worker/scripts/r2-lifecycle.ts` applies the rules (dry run by default; `--apply` reads the configuration back afterwards). **⛔ WHAT IS STILL OPEN: no rule exists on the bucket, so the sentence on the legal pages is still false today.** The dry run was executed against the real bucket and returned `AccessDenied` — the R2 token in `.env` is object-scoped, and lifecycle is a bucket-level operation needing **Admin Read & Write**. Two ways to close it: issue such a token and run the script with `--apply`, or enter six 30-day rules by hand in the Cloudflare dashboard (R2 → bucket → Settings → Object lifecycle rules), one per prefix — and **`previews/` must not be among them**. Note `--apply` deletes every object older than 30 days under those prefixes IRREVERSIBLY, including the stranded `renders/lambda-dqbz7jwul1.mp4`. Original row: |
| ~~❌ ⛔~~ | **30-day retention — the legal pages PROMISE it** | 👤 sets the R2 lifecycle rule; 🤖 does the expired-asset UI | `/uslovi` §8 and `/privatnost` §4 both state files are deleted automatically after 30 days ("brišu se automatski i ne mogu se povratiti"). No lifecycle rule exists on the bucket, so today that sentence is false. Must be true before the first real user — promoted to ⛔ 2026-08-18 |
| ✅ | **Server-log retention ≤30 days — `/privatnost` §4 now claims it** | — | **DONE + VERIFIED live 2026-08-19** (`9078a74`): `logging: json-file, max-size 10m × max-file 3` on all four services via a YAML anchor, containers recreated on the VPS, `docker inspect` confirms the LogConfig on all three running containers, stack healthy after. Honest caveat recorded in the compose comment: this bounds SIZE (30 MB/container — rotates out within days at our volume), not strictly AGE; if a regulator ever demands age-based deletion, that is a logrotate cron, not compose |
...
| Status | Check | Who | Note |
|---|---|---|---|
| ✅ | **VPS reboot drill** | — | **DONE 2026-08-19** (owner sent the reboot — the session classifier refuses it from Claude — Claude verified recovery): all three containers came back `healthy` on their own within ~1 min of boot, worker re-attached to both queues on REAL providers, and HTTP matched the pre-reboot baseline exactly (`/` 200, robots 200, traversal 401 → signing branch live). Bonus proof: the worker logged `"shutting down" signal=SIGTERM` during the reboot — the 2026-08-14 graceful-drain fix passed its first real machine restart. The `.env not found` line in worker logs is benign (`--env-file-if-exists`; env comes from compose) |
...
| 🟡 | **Database backup + restore drill** | 👤 then 🤖 | **First DATA backup ever taken 2026-08-19**: all five tables exported via PostgREST + service key, row counts verified against the server's exact counts (assets 4, jobs 5, profiles 2, ledger 7, holds 0), stored OUTSIDE the repo at `D:\Projekti\_backups\adgen-db-export-2026-08-19\` (schema itself lives in git as migrations 0001–0011). Still missing: a real `pg_dump` (pg 16 IS installed on the second machine, but the DB password is only in the Supabase dashboard — no env file carries it) and a restore rehearsal. A wiped table on free tier is still GONE — this covers 18 rows of today, not tomorrow |
...
| 🟡 | **Disk-full + uptime monitoring — WRITTEN, not installed** | 👤 runs 4 commands | Build cache has filled the disk three times (33%→72% once); nothing alarms. No uptime check on `/` either — a down site is currently discovered by a visitor. **`infra/watchdog.sh` + `watchdog.service` + `watchdog.timer` now exist** (2026-08-20): every 15 min it checks disk %, that `/` answers 200, and that all three containers are running+healthy; it reports to `ALERT_WEBHOOK_URL` if that is set and to the journal always, and exits non-zero so `systemctl list-units --failed` shows incidents without parsing log text. It restarts and prunes NOTHING deliberately. **VERIFIED on the real box, both branches** — happy path printed `ok: disk 44%, site 200, three containers healthy` (exit 0), and a forced failure printed the disk and site lines with exit 1; the dry-run copy was deleted afterwards. Not installed: that is a standing change to the box, so the four install commands are in the script header for the owner. ⚠️ Without `ALERT_WEBHOOK_URL` it can only write to the journal — the alerting row above is still the blocker |
...
| ✅ | **Pin yt-dlp + add Dependabot** | — | Both done 2026-08-19: `apps/web/Dockerfile` pins `2026.07.04` — the tag `releases/latest` resolved to that same day, so the deployed image already runs this exact version and the pin changes nothing until someone bumps it; the pinned URL answers 200 (HEAD-checked, 3.07 MB). `.github/dependabot.yml` covers npm (weekly, minor+patch grouped), both Dockerfiles and github-actions. **Closed 2026-08-20:** two real deploys that day rebuilt both images, so the pinned curl line has now been executed, and the LIVE web container answers `yt-dlp --version` with exactly `2026.07.04` from `youtube-dl-exec@3.1.9/bin/yt-dlp` — checked inside the running container, not inferred from the Dockerfile |
| ❌ | **A retention rule that actually deletes a file** | 👤 then 🤖 | Added 2026-08-22 with `31189ec`. The app now answers 410 for an expired file and „Moje reklame" says so, but **no lifecycle rule exists on the bucket** (the token cannot write one — see §5), so nothing has ever been deleted on schedule and no customer-facing expiry has ever been OBSERVED, only unit-tested. Two things to check the first time a rule is live: an object under an expiring prefix actually disappears at 30 days, and an object under `previews/` does NOT |
| ❌ | **`--apply` has never been run against the real bucket** | 👤 decides | Same row as §5. The write path (`putLifecycleRules`) is CODE-COMPLETE and has never been executed — the read path was executed and returned `AccessDenied`, which is the only live evidence either function has produced |
